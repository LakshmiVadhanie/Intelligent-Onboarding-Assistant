...
# logging: allow override via env var DEBUG_TESTS=1
_log_level = logging.DEBUG if os.getenv("DEBUG_TESTS") else logging.WARNING
logging.basicConfig(level=_log_level)
logger = logging.getLogger(__name__)

# GCS loader import for optional GCS-first loading
...
def test_data_loading():
    print_header("TEST 1: DATA LOADING (GCS)")

    try:
        from src.data.load_from_gcs import GCSDataLoader

        if not GCS_KEY_PATH:
            raise ValueError("GCS_KEY_PATH not found in .env file")
        ...
        chunks, counts = loader.load_all_chunks()

        assert len(chunks) > 0, "No data loaded from GCS!"

        print(f" PASSED: Loaded {len(chunks)} chunks from GCS")
...
def test_rag_generation():
    ...
    try:
        ...
        if api_key and getattr(rag, "client", None):
...
if __name__ == "__main__":
    ...
    # sys.exit(0 if success else 1)  # uncomment if you want exit codes for CI
    if not success:
        print("\nOne or more tests failed. Check logs above for details.")
